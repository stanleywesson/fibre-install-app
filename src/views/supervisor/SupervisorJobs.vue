  ...
  <div>
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      <div class="px-4 py-6 sm:px-0">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">Company Jobs</h1>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="hidden lg:table-cell px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Installer</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="job in filteredJobs" :key="job.id">
                ...
                <td class="hidden lg:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {{ job.assignedInstallerId ? usersStore.users.find(u => u.id === job.assignedInstallerId)?.name : 'Unassigned' }}
                </td>
                <td class="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {{ job.scheduledDate ? new Date(job.scheduledDate).toLocaleDateString() : '-' }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <RouterLink :to="`/supervisor/jobs/${job.id}`" class="text-blue-600 hover:text-blue-900">
                    View Details
                  </RouterLink>
                </td>
              </tr>
